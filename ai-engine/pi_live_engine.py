...
def main():
    parser = argparse.ArgumentParser(description="STIS Raspberry Pi Live Engine")
    ...
    # Clean previous data to ensure a fresh session
    try:
    ...
    print("\n" + "="*50)
    print(" 🚀 STIS LIVE SYSTEM - RASPBERRY PI DEPLOYMENT")
    print("="*50)
    print(f" Processing: {args.video}")
    print(" STATUS: RUNNING (Broadcasting to Dashboard)")
    print("="*50 + "\n")
    ...
    try:
        while cap.isOpened():
            ...
            frame_count += 1
            if frame_count % 2 != 0: continue # Process every 2nd frame for RasPi performance
            ...
            for box in results.boxes:
                ...
                if point_in_polygon((cx, cy), ZONE_POLYGON):
                    vehicle_counts += 1
                    # Mock speed based on history (for demo)
            ...
            if frame_count % UPLOAD_INTERVAL_FRAMES == 0:
                ...
                try:
                    ...
                    # 📺 Terminal Dashboard (TV Optimized)
                    status_marker = "🟢 OK" if density < 0.6 else "🟠 BUSY" if density < 0.8 else "🔴 CRITICAL"
                    print(f"\r[{status_marker}] Vehicles: {vehicle_counts:2d} | Density: {density*100:4.1f}% | Prediction: {predicted_density*100:4.1f}% | Sync: CLOUD ✅", end="")
                except Exception as e:
                    print(f"\n[ERROR] Sync failed: {e}")
